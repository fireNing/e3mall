...
function isOnline(wwwurl,homeurl,passporturl){
	$.getJSON( wwwurl+"/ajax/isOnline/?callback=?", function( data ) {
		if (data.welcome){
			...
			$('#login').html(welComeMsg+''+passporturl+'<a href="/src/main/webapp?returnUrl=">请登录</a> | <a href="'+escape(document.location.href)+''+passporturl+'/reg/?returnUrl='+escape(document.location.href)+'">免费注册</a>');
		}
		if(data.qqcb){
			$('#qqcb').html(data.qqcb); 
		}
